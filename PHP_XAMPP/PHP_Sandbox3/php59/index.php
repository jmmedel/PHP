...
<html>
  ...
  <body>
    ...
    if (preg_match("/Daniel/", $string, $match)) {
    ...
    if (preg_match_all("/Daniel/", $string, $match)) {
      print_r($match);
    }
    //Here you will notice that our print_r has found two results, and put them in an array.

    //Have you noticed that our array with results is inside another array? This is because we can do multiple searches in a single expression. Each search will then appear in a sepparate array.

    //Here is an example:
    if (preg_match_all("/Da(ni)el/", $string, $match)) {
      print_r($match);
    }
    //As you may notice, the print_r outputs two arrays because we searched for "Daniel" AND "ni".

    //If we want to replace characters within a string, we can use preg_replace()!
    $stringNew = preg_replace("/Daniel/", "John", $string);
    echo $stringNew;
